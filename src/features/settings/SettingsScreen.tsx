...
export function SettingsScreen({ user, shift, onLogout }: {
  ...
  const logoutLocked = useMemo(() => {
    const total = transactions.reduce((s, t) => s + t.finalAmount, 0);
    return total > 0;
  }, [transactions]);
  ...
  const signOut = async () => { await api.logout(); setLogout(false); onLogout(); };
  ...
  return <ScreenShell>
    ...
    {logoutLocked ? <Text style={styles.error}>Remit all pending collections before logging out.</Text> : null}
    <Pressable disabled={logoutLocked} style={[styles.button, { backgroundColor: "#9F1239", opacity: logoutLocked ? .35 : 1 }]} onPress={() => setLogout(true)}><Text style={[styles.buttonText, { color: "#fff" }]}>Log Out</Text></Pressable>
