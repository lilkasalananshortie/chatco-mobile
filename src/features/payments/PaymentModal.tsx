...
export function PaymentModal({ visible, shift, isOnline, onClose, onSaved }: {
  ...
  useEffect(() => {
    ...
    loadFareMatrix();
    void api.receiptSettings().then(setReceiptSettings).catch(() => undefined);
    ...
    <ModalShell visible={visible} title="Collect Payment" onClose={close}>
        ...
        <>
          ...
          {!isOnline ? (
            <Text style={styles.error}>
              Offline mode: cash fares are saved and will sync automatically. GCash and vouchers are disabled.
            </Text>
          ) : null}
          <MethodCard
            title="Cash Payment"
            detail="Calculate fare and record cash collection"
            onPress={() => chooseMethod("CASH")}
          />
          <MethodCard
            title="GCash Payment"
            detail={isOnline ? "Generate a binding QR and track payment status" : "Unavailable offline — reconnect to generate a QR"}
            onPress={() => chooseMethod("GCASH")}
            disabled={!isOnline}
          />
          <MethodCard
            title="Voucher / Free Ride"
            detail={isOnline ? "Validate a commuter voucher code" : "Unavailable offline — reconnect to validate a voucher"}
            onPress={() => chooseMethod("VOUCHER")}
            disabled={!isOnline}
          />
        </>
      ) : null}
      ...
      {step === "route" ? (
        <>
          ...
          <TextInput
            ...
            style={styles.input}
            placeholder="Search point or landmark"
            placeholderTextColor={colors.muted}
          />
          {pickup ? (
            <Pressable style={styles.card} onPress={() => setSelecting("pickup")}>
              <Text style={styles.label}>Pickup</Text>
              <Text style={styles.cardTitle}>{pickupName}</Text>
            </Pressable>
          ) : null}
          {dropoff ? (
            <Pressable style={styles.card} onPress={() => setSelecting("dropoff")}>
              <Text style={styles.label}>Drop-off</Text>
              <Text style={styles.cardTitle}>{dropoffName}</Text>
            </Pressable>
          ) : null}
          {!pickup || !dropoff ? (
            filtered.map((point) => {
              const uniqueSubStops = [...point.landmarks, ...(point.subStops ?? [])].filter(
                (item, index, all) => all.indexOf(item) === index
              );
              const isSearching = Boolean(search.trim());
              const isExpanded =
                expandedPoint === point.pointNumber ||
                (isSearching && uniqueSubStops.some((l) => l.toLowerCase().includes(search.trim().toLowerCase())));
              return (
                <View key={point.pointNumber} style={styles.card}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                    <Pressable style={{ flex: 1 }} onPress={() => choosePoint(point)}>
                      <Text style={styles.cardTitle}>{point.name}</Text>
                      <Text style={styles.subtitle}>{point.code} · Main stop</Text>
                    </Pressable>
                    {uniqueSubStops.length > 0 ? (
                      <Pressable
                        onPress={() => setExpandedPoint(isExpanded ? null : point.pointNumber)}
                        style={[
                          styles.button,
                          styles.secondaryButton,
                          { minHeight: 32, paddingVertical: 4, paddingHorizontal: 10, alignSelf: "center" },
                        ]}
                      >
                        <Text style={[styles.buttonText, styles.secondaryButtonText, { fontSize: 11 }]}>
                          {isExpanded ? "Hide stops" : `+${uniqueSubStops.length} sub-stops`}
                        </Text>
                      </Pressable>
                    ) : null}
                  </View>
                  {isExpanded && uniqueSubStops.length > 0 ? (
                    <View style={{ marginTop: 8, gap: 6, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 8 }}>
                      <Text style={[styles.subtitle, { fontSize: 11 }]}>Sub pickup / drop-off points:</Text>
                      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                        {uniqueSubStops.map((landmark) => (
                          <Pressable
                            key={landmark}
                            style={[
                              styles.button,
                              styles.secondaryButton,
                              { minHeight: 34, paddingVertical: 6, paddingHorizontal: 10 },
                            ]}
                            onPress={() => choosePoint(point, `${point.name} · ${landmark}`)}
                          >
                            <Text style={[styles.buttonText, styles.secondaryButtonText, { fontSize: 11 }]}>{landmark}</Text>
                          </Pressable>
                        ))}
                      </View>
                    </View>
            ...
            <>
                    ...
                    {([false, true] as const).map((value) => (
                      <Pressable
                        ...
                        style={[
                          styles.button,
                          styles.secondaryButton,
                          { flex: 1, backgroundColor: groupMode === value ? colors.primary : colors.surface2 },
                        ]}
                        onPress={() => setGroupMode(value)}
                      >
                        <Text style={styles.buttonText}>{value ? "Multiple" : "Single"}</Text>
                      </Pressable>
                    ))}
                  </View>
                </>
              ...
              {groupMode ? (
                <View style={styles.card}>
                  <Text style={styles.label}>
                    {method === "GCASH" ? "Companions (payer added after scan)" : "Passengers in this group"}
                  </Text>
                  {(
                    [
                      ["REGULAR", "Regular"],
                    ...
                    >
                      ...
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                        <Pressable
                          onPress={() =>
                            setGroupCounts((current) => ({ ...current, [type]: Math.max(0, current[type] - 1) }))
                          }
                        >
                        ...
                        <Pressable
                          onPress={() =>
                            setGroupCounts((current) => ({ ...current, [type]: Math.min(50, current[type] + 1) }))
                          }
                        >
                          <Text style={styles.cardTitle}>+</Text>
                        </Pressable>
                  ...
                  <Text style={styles.subtitle}>
                    {groupCompanionCount} {method === "GCASH" ? "companion" : "passenger"}
                    {groupCompanionCount === 1 ? "" : "s"} entered
                    {method === "GCASH" ? " · the scanning payer is added automatically" : ""}
                  </Text>
                    ...
                    {types.map((type) => (
                      <Pressable
                        ...
                        style={[
                          styles.button,
                          styles.secondaryButton,
                          commuterType === type.id && { backgroundColor: colors.primary },
                        ]}
                        onPress={() => setCommuterType(type.id)}
                      >
                        <Text style={[styles.buttonText, styles.secondaryButtonText]}>{type.label}</Text>
                      </Pressable>
                    ))}
                  </View>
                </>
              ...
              {method !== "GCASH" && !groupMode ? (
                ...
                <Text style={styles.subtitle}>
                  Fare and commuter discount will be confirmed after the commuter scans the QR code.
                </Text>
              ) : null}
              <Pressable
                disabled={busy}
                style={styles.button}
                onPress={() => (method === "GCASH" ? void submit() : setStep("confirm"))}
              >
                <Text style={styles.buttonText}>
                  {method === "GCASH" ? (busy ? "Generating..." : "Generate QR Code") : "Continue"}
                </Text>
              </Pressable>
            </>
          )}
          <Pressable style={[styles.button, styles.secondaryButton]} onPress={() => setStep("method")}>
            <Text style={[styles.buttonText, styles.secondaryButtonText]}>Back to methods</Text>
          </Pressable>
        </>
      ) : null}
        ...
        <>
          ...
          <View style={styles.card}>
            ...
            <Row
              ...
              value={
                groupMode
                  ? `${groupCompanionCount} passengers`
                  : method === "GCASH"
                  ? "Detected after scan"
                  : commuterType
              }
          ...
          <Pressable style={[styles.button, styles.secondaryButton]} onPress={() => setStep("route")}>
            <Text style={[styles.buttonText, styles.secondaryButtonText]}>Edit details</Text>
          </Pressable>
        </>
      ) : null}
      ...
      {step === "qr" && gcash ? (
        <View style={{ alignItems: "center" }}>
          ...
          <View style={{ backgroundColor: "#fff", padding: 14, marginVertical: 18, borderRadius: 14 }}>
            <QRCode value={gcash.qrToken} size={210} />
          </View>
          <Text style={styles.subtitle}>Expires {new Date(gcash.expiresAt).toLocaleTimeString()}</Text>
          {gcash.checkoutUrl ? (
            <Pressable
              style={[styles.button, { alignSelf: "stretch" }]}
              onPress={() => void Linking.openURL(gcash.checkoutUrl!)}
            >
              <Text style={styles.buttonText}>Open checkout link</Text>
            </Pressable>
          ) : null}
          {error ? <Text style={[styles.error, { marginVertical: 8 }]}>{error}</Text> : null}
          <Pressable
            disabled={busy}
            style={[styles.button, styles.secondaryButton, { alignSelf: "stretch" }]}
            onPress={() => void cancelGcash()}
          >
            <Text style={[styles.buttonText, styles.secondaryButtonText]}>
              {busy ? "Cancelling..." : "Cancel pending payment"}
            </Text>
          </Pressable>
        </View>
      ) : null}
        ...
        <>
          ...
          {receiptTransactions.length > 0 ? (
            <TransactionReceipt
              ...
              transactionId={receiptTransactions[0]?.transactionId}
              timestamp={receiptTransactions[0]?.timestamp ?? Date.now()}
              unitNumber={receiptTransactions[0]?.unitNumber || shift.unitNumber || ""}
              conductorName={receiptTransactions[0]?.conductorName || shift.conductorName || ""}
              passengerType={receiptTransactions[0]?.passengerRole || commuterType}
              passengerName={receiptTransactions[0]?.passengerName}
              from={receiptTransactions[0]?.from || pickupName}
              to={receiptTransactions[0]?.to || dropoffName}
              baseFare={receiptTransactions[0]?.baseFare ?? (fare?.regular ?? 0)}
              discountAmount={receiptTransactions[0]?.discountAmount ?? (fare?.discount ?? 0)}
              finalFare={
                receiptTransactions.reduce((s, t) => s + t.finalAmount, 0) ||
                (fare ? (groupMode ? groupFare : fare.final) : 0)
              }
              paymentMethod={method === "GCASH" ? "GCash" : method === "VOUCHER" ? "Voucher" : "Cash"}
              receiptQrToken={receiptTransactions[0]?.qrToken}
              multiplePaymentReference={receiptTransactions[0]?.multiplePaymentReference}
              driverName={receiptTransactions[0]?.driverName || shift.driverName}
              totalPassengers={groupMode ? groupCompanionCount : 1}
              grossFare={groupMode ? groupFare : fare?.regular}
              passengerBreakdown={
                groupMode
                  ? groupPassengers.map((row) => ({
                      passengerType: row.type,
                      quantity: row.quantity,
                      subtotal:
                        row.quantity * (row.type === "REGULAR" ? (fare?.regular ?? 0) : (fare?.discounted ?? 0)),
                    }))
                  : undefined
              }
            />
      ...
      {step === "failed" ? (
        <>
          ...
          <Pressable style={[styles.button, styles.secondaryButton]} onPress={close}>
  ...
  function MethodCard({
    ...
    return (
      <Pressable disabled={disabled} style={[styles.card, disabled ? { opacity: 0.5 } : null]} onPress={onPress}>
  ...
  function Row({ label, value }: { label: string; value: string }) {
    return (
      <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 16, marginVertical: 6 }}>
        <Text style={styles.subtitle}>{label}</Text>
        <Text style={[styles.cardTitle, { flex: 1, textAlign: "right" }]}>{value}</Text>
      </View>
    );
  }
}
